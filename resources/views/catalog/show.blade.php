...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-600 mb-8">
        ...
        <a href="{{ route('catalog.index') }}" class="hover:text-red-600 transition-colors">Katalog</a>
        <i class="fas fa-chevron-right text-xs"></i>
        <span class="text-red-600 font-medium">{{ $produk->nama_produk }}</span>
    </nav>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8 lg:p-12">
            ...
            <div class="space-y-4">
                <div class="relative group overflow-hidden rounded-2xl bg-gray-100">
                    <img src="{{ $produk->gambar ? asset('storage/' . $produk->gambar) : 'https://via.placeholder.com/600x400' }}" 
            ...
            <div class="space-y-6">
                {{-- Title --}}
                <div>
                    <h1 class="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">{{ $produk->nama_produk }}</h1>
                    ...
                    <div class="bg-gradient-to-br from-red-50 to-red-100 p-4 rounded-xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-900">{{ $produk->levelPedas->nama_level ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-purple-50 to-purple-100 p-4 rounded-xl">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="font-semibold text-gray-900">{{ $produk->kategori->nama_kategori ?? '-' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gradient-to-r from-red-50 via-orange-50 to-yellow-50 p-6 rounded-2xl border-2 border-red-100">
                    ...
                    <div class="flex items-end gap-3">
                        ...
                        <span class="text-sm text-gray-500 line-through mb-2">
                            Rp {{ number_format($produk->harga * 1.2, 0, ',', '.') }}
                        </span>
                    </div>
                ...
                <div class="bg-gray-50 p-4 rounded-xl border border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            <i class="fas fa-box text-2xl {{ $produk->stok > 0 ? 'text-green-600' : 'text-red-600' }}"></i>
                            <div>
                                <p class="text-sm text-gray-600">Ketersediaan Stok</p>
                                <p class="font-bold {{ $produk->stok > 0 ? 'text-green-600' : 'text-red-600' }}">
                ...
                @auth
                    @if(auth()->user()->isCustomer())
                        @if($produk->stok > 0)
                            <form action="{{ route('cart.add', $produk) }}" method="POST" class="space-y-4">
                                @csrf
                                <div class="flex items-center gap-4">
                                    <div class="flex-shrink-0">
                                        ...
                                        <div class="flex items-center border-2 border-gray-200 rounded-xl overflow-hidden">
                                            ...
                                            <input type="number" id="quantity" name="quantity" value="1" min="1" max="{{ $produk->stok }}" 
                    ...
                    @endif
                @else
                    <a href="{{ route('login') }}" class="block w-full text-center bg-gradient-to-r from-red-600 to-orange-600 text-white py-4 rounded-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300 font-bold text-lg">
    ...
    <div class="mb-12">
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="{{ route('catalog.index') }}" class="text-red-600 hover:text-red-700 font-semibold flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border border-gray-100">
                ...
                <div class="p-4">
                    ...
                    <a href="{{ route('catalog.show', $product) }}" class="block text-center bg-gradient-to-r from-red-600 to-orange-600 text-white py-2.5 rounded-xl hover:shadow-xl transform hover:scale-105 transition-all duration-300 font-semibold">
